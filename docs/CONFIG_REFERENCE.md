...
国内医疗政策、协会与行业媒体统一登记在 `config/sources.yml`，不需要新增 Secret。
...
稳定错误类别：`unsupported_parser_profile`、`request_failed`、`response_too_large`、`unexpected_content_type`、`no_valid_items`、`invalid_json_shape`、`invalid_item_url`、`invalid_publish_time`。单一来源失败只更新该来源状态，不终止其他来源。
