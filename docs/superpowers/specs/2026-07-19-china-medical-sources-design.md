...
| `cn-healthcare` | 健康界 | `https://www.cn-healthcare.com/?logo=1` | `social_observation` / `html_list` | `primary_care` | B | 4 |
...
| `cn-bioon` | 生物谷 | `https://www.bioon.com/BioMedical` | `social_observation` / `html_list` | `pharma_device` | C | 3 |
...
- 医师报：GitHub Runner 的 HTTPS 与移动入口持续网络失败，HTTP 入口返回 444；
...
医学界和健康界额外过滤纯病例讨论、患者用药、疾病症状和单纯诊疗指南；生物谷使用官方医药产业专区，优先保留创新药械、产业、审批、AI 和科研转化，降低纯基础机制论文的比例。
...
2. 医趋势和医师报均未进入最终清单。医趋势最新焦点区没有列表级发布日期；医师报在本机可访问，但 GitHub Runner 对其 HTTPS/移动入口持续出现 `network_error`、HTTP 入口返回 444。最终替换为健康界公开列表，保留明确发布日期，并通过 GitHub Actions 可达性检查。
