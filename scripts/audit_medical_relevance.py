#!/usr/bin/env python3
"""Generate a lightweight audit report for medical relevance scoring output."""
    ...
    for item in items[:limit]:
        rows.append(
            "| {score:.2f} | {label} | {site} | {source} | {title} | {reason} |".format(
                ...
                label=str(item.get("medical_label") or ""),
                ...
                reason=str(item.get("medical_relevance_reason") or ""),
            )
        )
    return rows
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Audit medical relevance scoring output")
    ...
    kept_raw = [item for item in raw if item.get("medical_is_related")]
    dropped = [item for item in raw if not item.get("medical_is_related")]

    label_counts = collections.Counter(str(item.get("medical_label") or "unknown") for item in kept_raw)
    ...
    review_band = [
        item
        for item in raw
        if 0.45 <= float(item.get("medical_score") or 0) < float(latest.get("medical_relevance_threshold") or 0.65)
    ]
    high_drops = sorted(dropped, key=lambda item: float(item.get("medical_score") or 0), reverse=True)
    top_kept = sorted(kept_raw, key=lambda item: float(item.get("medical_score") or 0), reverse=True)

    lines = [
        "# Medical Relevance Audit — v0.4.0",
        "",
        "## Summary",
        "",
        f"- generated_at: `{latest.get('generated_at')}`",
        f"- topic_filter: `{latest.get('topic_filter')}`",
        f"- threshold: `{latest.get('medical_relevance_threshold')}`",
    ...
    lines.extend(table_rows(sorted(review_band, key=lambda item: float(item.get("medical_score") or 0), reverse=True), args.sample_size))
    lines.append("")
