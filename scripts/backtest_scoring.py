...
medical_relevance and report exactly what a scoring change does before it ships.
...
from scripts.medical_relevance import score_medical_relevance as candidate_scorer  # noqa: E402
...
def load_scorer_from_rev(rev: str):
    source = subprocess.run(
        ["git", "show", f"{rev}:scripts/medical_relevance.py"],
        cwd=REPO_ROOT,
        capture_output=True,
        text=True,
        check=True,
    ).stdout
    with tempfile.NamedTemporaryFile("w", suffix="_medical_relevance_baseline.py", delete=False) as fh:
        fh.write(source)
        path = fh.name
    spec = importlib.util.spec_from_file_location("medical_relevance_baseline", path)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module.score_medical_relevance
...
def compare_scoring(items: list[dict], baseline_fn, candidate_fn, sample_cap: int = 30) -> dict:
    ...
    for record in items:
        ...
        if base["is_medical_related"]:
            kept_baseline += 1
            site_keep_baseline[site] += 1
        if cand["is_medical_related"]:
            kept_candidate += 1
            site_keep_candidate[site] += 1
        if base["is_medical_related"] != cand["is_medical_related"]:
            entry = {
                "site_id": site,
            ...
            (flips_to_drop if base["is_medical_related"] else flips_to_keep).append(entry)
        elif base["is_medical_related"] and base["label"] != cand["label"]:
...
def render_markdown(report: dict, days: int, baseline_rev: str) -> str:
    lines = [
        ...
        f"- Flips medical->not_medical: **{report['flips_to_drop_count']}** | not_medical->medical: **{report['flips_to_keep_count']}**",
        "",
        "## Flip samples (medical -> not_medical)",
        "",
    ]
    ...
    lines += ["", "## Flip samples (not_medical -> medical)", ""]
...
def main() -> int:
    ...
    print(f"Flips medical->not_medical: {report['flips_to_drop_count']} | not_medical->medical: {report['flips_to_keep_count']}")
