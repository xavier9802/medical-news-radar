...
Use this when evaluating a new information source for Medical News Radar.
...
| social media public timeline | Curated central feed or optional social API adapter | Low as default |
...
- Read a curated central feed that already uses official social API, like
  medical feed aggregators.
- Add a self-hosted optional X adapter using `X_BEARER_TOKEN`; skip cleanly when
  the token is missing.

For optional social API adapters:
...
pytest -q
python scripts/update_news.py --output-dir /tmp/medical-news-radar-data --window-hours 24 --archive-days 21
```

Inspect `/tmp/medical-news-radar-data/source-status.json`:

- source appears with `ok: true`
- `failed_sites` is empty or the failure is expected and documented
- item count is plausible
- medical-focused view is not flooded with off-topic items
