...
def test_normalize_title_for_overlap_removes_noise_words_and_punctuation():
    assert normalize_title_for_overlap("FDA approves new cancer drug — HealthTech Hub") == "fda approves new cancer drug"
    assert normalize_title_for_overlap("[Exclusive] NEJM: new diabetes study! 2026") == "nejm new diabetes study"


def test_title_similarity_handles_reordered_tokens():
    assert title_similarity("FDA approves new oncology drug", "New oncology drug approved by FDA") >= 0.88


def test_classify_overlap_matches_exact_url_after_normalization():
    candidate = record("candidate", "Different title", "https://fda.gov/news/oncology-drug?utm_source=x#section")
    existing = record("official_health", "FDA oncology drug update", "https://fda.gov/news/oncology-drug")

    match = classify_overlap(candidate, existing)

    assert match is not None
    assert match["match_type"] == "url_exact"
    assert match["matched_site_id"] == "official_health"
    assert match["score"] == 1.0


def test_classify_overlap_matches_similar_titles_with_different_urls():
    candidate = record("candidate", "FDA approves a new oncology drug", "https://candidate.example/a")
    existing = record("official_health", "FDA approves new oncology drug", "https://fda.gov/news/oncology-drug")
...
def test_evaluate_source_overlap_counts_duplicates_uniques_and_top_sources():
    candidate_items = [
        record("candidate", "FDA approves a new oncology drug", "https://candidate.example/a"),
        record("candidate", "WHO releases vaccination workflow", "https://candidate.example/b"),
        record("candidate", "Unique hospital IT benchmark", "https://candidate.example/c"),
    ]
    baseline_items = [
        record("official_health", "FDA approves new oncology drug", "https://fda.gov/news/oncology-drug"),
        record("healthtech_hub", "WHO releases vaccination workflow", "https://healthtech.example/who"),
        record("community", "Unrelated medical post", "https://community.example/u"),
    ]
